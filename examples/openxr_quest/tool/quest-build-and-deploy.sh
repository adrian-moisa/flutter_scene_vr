...
quest_flutter="${QUEST_FLUTTER:-flutter}"
if ! command -v "$quest_flutter" >/dev/null 2>&1; then
	printf 'Flutter is unavailable: %s\n' "$quest_flutter" >&2
	exit 1
fi
...
quest_perf_logs="${QUEST_PERF_LOGS:-0}"
...
quest_step 2 "Resolving Flutter dependencies"
"$quest_flutter" pub get

quest_step 3 "Building the ARM64 $quest_build_mode APK"
quest_flutter_build_args=(
	build apk "--$quest_build_mode" --target-platform android-arm64
)
...
printf '\nFlutter Scene OpenXR deployment complete.\n'
printf 'Device: %s\n' "$quest_serial"
printf 'APK: %s\n' "$quest_apk"
